...
//                Matcher matcher = pattern.matcher(s);
                ...
                if(qq!=0) {
                    try {
                        msg = new At(qq).plus("今天的老婆是「").plus(waifus.get(getDailyHash(qq, 405))).plus("」!");
                    } catch (NoSuchAlgorithmException e) {
                        throw new RuntimeException(e);
                    }
                    g.getGroup().sendMessage(msg);
...
//                Matcher matcher = pattern.matcher(s);
                ...
                if(qq!=0) {
                    try {
                        msg = new At(qq).plus("今天的宝可梦是「").plus(pokemons.get(getDailyHash(qq, 1012))).plus("」!");
                    } catch (NoSuchAlgorithmException e) {
                        throw new RuntimeException(e);
                    }
                    g.getGroup().sendMessage(msg);
